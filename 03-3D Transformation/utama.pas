...
type
  ...
  TFormUtama = class(TForm)
    ...
    RBParalel: TRadioButton;
    RBPerspektif: TRadioButton;
...
var
  ...
  theta, rad: Double;
begin
  theta:= StrToFloat(EditTheta.Text);
  rad:= theta*PI/180;
  if RBX.Checked = True then
  begin
    for i:= 1 to 8 do
    begin
      temp:= P[i].y;
      P[i].y:= P[i].y * Cos(rad) - P[i].z * Sin(rad);
      P[i].z:= temp * Sin(rad) + P[i].z * Cos(rad);
      P[i].x:= P[i].x;
    end;
  end
  ...
  begin
    ...
    begin
      temp:= P[i].x;
      P[i].x:= P[i].x * Cos(rad) - P[i].z * Sin(rad);
      P[i].z:= temp * Sin(rad) + P[i].z * Cos(rad);
      P[i].y:= P[i].y;
    end;
  end
  else if RBZ.Checked = True then
  begin
    for i:= 1 to 8 do
    begin
      temp:= P[i].x;
      P[i].x:= P[i].x * Cos(rad) - P[i].y * Sin(rad);
      P[i].y:= temp * Sin(rad) + P[i].y * Cos(rad);
      P[i].z:= P[i].z;
    end;
  end;
  ButtonHapusClick(Sender);
  if RBParalel.Checked = True then
  begin
     ButtonParalelClick(Sender)
  end
  else if RBPerspektif.Checked = True then
  begin
     ButtonPerspektifClick(Sender);
  end;
end;
...
begin
  ...
  begin
    tempX:= TEdit(FormUtama.FindComponent('EditP' + IntToStr(i) + 'X'));
    tempX.Text:= FloatToStr(0);

    tempY:= TEdit(FormUtama.FindComponent('EditP' + IntToStr(i) + 'Y'));
    tempY.Text:= FloatToStr(0);

    tempZ:= TEdit(FormUtama.FindComponent('EditP' + IntToStr(i) + 'Z'));
    tempZ.Text:= FloatToStr(0);

    tempXP:= TEdit(FormUtama.FindComponent('EditP' + IntToStr(i) + 'XP'));
    tempXP.Text:= IntToStr(0);

    tempYP:= TEdit(FormUtama.FindComponent('EditP' + IntToStr(i) + 'PY'));
    tempYP.Text:= IntToStr(0);
  end;
end;
